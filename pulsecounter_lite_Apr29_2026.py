...
Change from v6:
  - Block read multiplier reduced from 4 → 2 (BLOCK_SIZE * 2).
    v6 still showed deterministic ~2x PRI gaps (one missed pulse every few
    cycles) indicating block boundary alignment was still occasionally causing
    a pulse to be dropped.  The gap size was always ~2x PRI and never random,
    ruling out general CPU overload.  Halving the read size again reduces the
    probability that a given pulse falls exactly at a block boundary, and
    shortens the blocking read call so the loop stays current with the stream.

    The --read-mult argument is exposed so this can be tuned further if needed
    without editing the script.  If gaps persist at --read-mult 2, try 1.
    If CPU load becomes an issue on slower hardware, increase it.
...
parser = argparse.ArgumentParser(
    description=(
        "Pulse counter SDR logger — v7, tuned for PPM-based temperature estimation."
    )
)
...
parser.add_argument("--read-mult", type=int, default=2,
                    help=(
                        "Block read multiplier (default 2, i.e. read BLOCK_SIZE * 2 "
                        "samples per call). Lower values reduce the chance of a pulse "
                        "falling at a block boundary and shorten the blocking read, "
                        "keeping the loop current with the SDR stream. "
                        "Try 1 if ~2x PRI gaps persist. Increase on slower hardware "
                        "if CPU load is a concern."
                    ))
...
parser.add_argument("-m", "--minwidth", type=float, default=DEFAULT_MIN_WIDTH_MS,
                    help=(
                        "Minimum pulse width in ms (default 20.0 ms). "
...
parser.add_argument("--lockout-ms", type=float, default=500.0,
                    help=(
                        ...
                        "Rejects FIR tail ghosts at block boundaries. "
...
print(f"[Lockout]     {LOCKOUT_MS:.0f} ms cross-block duplicate suppression")
        ...
        for freq in EXPANDED_FREQ_LIST:
            ...
            while (time.time() - scan_start) < SCAN_TIME:
                ...
                for p in merged_peaks:
                    # Per-pulse timestamp from sample position
                    samples_from_end = n_dec - int(p)
                    pulse_time = block_time - datetime.timedelta(
                        seconds=samples_from_end / FS_DEC)

                    # Cross-block lockout gate
                    if LOCKOUT_MS > 0 and state["last_accepted_time"] is not None:
                        since_last_ms = (
                            pulse_time - state["last_accepted_time"]
                        ).total_seconds() * 1e3
                    ...
                    state["last_accepted_time"] = pulse_time

                    amp      = float(env[p])
                    peak_db  = 20.0 * np.log10(amp) if amp > 0 else -999.0
                    width_ms = estimate_width(env, p, FS_DEC) * 1e3

                    # Width filter — rejects partial block-boundary pulses
                    ...
                    if state["last_peak_time"] is None:
                        ...
                        delta    = pulse_time - state["last_peak_time"]
                    ...
                    state["last_peak_time"] = pulse_time
